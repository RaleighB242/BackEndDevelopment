...
<body>
  ...
  <section class="container my-2 bg-dark w-50 text-light p-2 studentsform">
    <form class="row g-3 p-3" action="DeleteSchedule.php" method="POST">
      <div class="col-12">
        <label for="TaID" class="form-label">Ta ID</label>
        <select class="form-control" name="TaID">
        <?php
        $sql = mysqli_query($link, "SELECT TaID, Ta_fname FROM TeachingAssistant");
        while ($row = $sql->fetch_assoc()){
         echo "<option value=".$row['TaID'].">".$row['Ta_fname']."</option>";
        }
        ?>
        </select>
      </div>
      ...
      <div class="col-12">
        ...
        <select class="form-control" name="TeachersID" required>
         <?php
         $sql = mysqli_query($link, "SELECT TeachersID, Teachers_fname FROM Teachers");
         while ($row = $sql->fetch_assoc()){
          echo "<option value=".$row['TeachersID'].">".$row['Teachers_fname']."</option>";
         }
         ?>
  ...
  <?php
		...
		if (isset($_POST['submit'])) {
			...
			$sql = "DELETE FROM Schedule WHERE TaID = '$TaID' AND TeachersID = '$TeachersID' ";
